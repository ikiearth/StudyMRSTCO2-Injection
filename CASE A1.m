...
% poro = dlmread('/Users/ikiearth/Documents/DATA KULIAH/SEMESTER 8/TUGAS AKHIR/NESR/mrst-2024b/TA NESR/porositas.csv');
% perm = dlmread('/Users/ikiearth/Documents/DATA KULIAH/SEMESTER 8/TUGAS AKHIR/NESR/mrst-2024b/TA NESR/permeabilitas.csv');
...
nstep   = 30 * day;         % Jumlah langkah waktu utama
...
injRate = 0.5 * sum(pv) / simTimeInj;    % Laju injeksi: fraksi volume pori dibagi total waktu simulasi
...
krW = flowfluid.krW(Sw);
krO = flowfluid.krO_wo(Sw);
...
W = verticalWell(W, G, rock, ...
    1, 1, 5:10, ...             % Lokasi: baris (I=1), kolom (J=10), perforasi penuh (K=[])
...
% Sumur Injeksi: I1
W = verticalWell(W, G, rock, ...
    30, 20, 10, ...             % Lokasi: baris (I=30), kolom (J=10), perforasi penuh (K=[])
    'name', 'Inj', ...          % Nama sumur
    'compi', [0 0 1], ...       % Komposisi fase: hanya gas CO2
    'val', injRate, ...         % Laju injeksi
...
% Alternatif: Gunakan simpleSchedule jika tidak perlu kontrol manual
%schedule = simpleSchedule(dt, 'W', W);

% Definisikan solver nonlinear untuk simulasi
nls = NonLinearSolver('useRelaxation', true); % Menggunakan relaksasi untuk stabilitas numerik

% Fungsi plotting setelah setiap timestep
fn = getPlotAfterStep(state0, model, schedule, 'plotWell', true);

model.extraStateOutput=true;

% Simulasi dengan kontrol yang telah dibuat
[~, states, report] = simulateScheduleAD(state0, model, schedule, ...
                                         'nonlinearsolver', nls, ... % Menggunakan solver nonlinear yang telah didefinisikan
                                         'afterStepFn', fn);         % Menjalankan plotting setelah setiap timestep
...
for t = 1:numSteps
    ...
    viscosityOilGrid(:, t) = muL ./ (centi * poise);
    viscosityGasGrid(:, t) = muV ./ (centi * poise);
...
muL_grid = viscosityOilGrid(:, numSteps);
plotCellData(G, muL_grid);
title('Distribusi Viskositas Minyak');
...
muV_grid = viscosityGasGrid(:, numSteps);
plotCellData(G, muV_grid);
title('Distribusi Viskositas Gas');
...
densityOil_grid = densityOilGrid(:, numSteps);
plotCellData(G, densityOil_grid);
title('Distribusi Densitas Oil');
...
densityGas_grid = densityGasGrid(:, numSteps);
plotCellData(G, densityGas_grid);
title('Distribusi Densitas Gas');
...
wellDataProd = struct(); % Untuk sumur produksi
wellDataInj = struct();  % Untuk sumur injeksi
...
for t = 1:numel(wellSols)               % Iterasi untuk setiap langkah waktu
    ...
    % Simpan data untuk sumur produksi
    for p = 1:numel(properties)
        propName = properties{p};
        if t == 1
            % Inisialisasi array untuk pertama kali
            wellDataProd.(propName) = [];
            wellDataInj.(propName) = [];
        end
        % Tambahkan data ke array
        wellDataProd.(propName) = [wellDataProd.(propName); ...
            arrayfun(@(idx) wellSols{t}(idx).(propName), prodIndices, 'UniformOutput', false)'];

        wellDataInj.(propName) = [wellDataInj.(propName); ...
            arrayfun(@(idx) wellSols{t}(idx).(propName), injIndices, 'UniformOutput', false)'];
    end
end
...
for p = 1:numel(properties)
    ...
    if ismember(propName, convertProps)
        wellDataProd_Field.(propName) = cellfun(@(x) abs(x) * SI_to_STB_per_day, wellDataProd.(propName), 'UniformOutput', false);
        wellDataInj_Field.(propName) = cellfun(@(x) x * SI_to_STB_per_day, wellDataInj.(propName), 'UniformOutput', false);
    elseif ismember(propName, convertPropsGas)
        wellDataProd_Field.(propName) = cellfun(@(x) abs(x) * SI_to_SCF_per_day, wellDataProd.(propName), 'UniformOutput', false);
        wellDataInj_Field.(propName) = cellfun(@(x) x * SI_to_SCF_per_day, wellDataInj.(propName), 'UniformOutput', false);
    elseif ismember(propName, convertPropsPressure)
        wellDataProd_Field.(propName) = cellfun(@(x) x * SI_to_psi, wellDataProd.(propName), 'UniformOutput', false);
        wellDataInj_Field.(propName) = cellfun(@(x) x * SI_to_psi, wellDataInj.(propName), 'UniformOutput', false);
    end
end

% Nama file Excel untuk menyimpan data
outputFilename = 'WellData-FieldCase1-05.xlsx';

% Konversi wellDataProd ke tabel
prodTable = struct2table(wellDataProd_Field);

% Konversi wellDataInj ke tabel
injTable = struct2table(wellDataInj_Field);

% Tulis data produksi ke sheet 1
writetable(prodTable, outputFilename, 'Sheet', 1, 'WriteVariableNames', true);

% Tulis data injeksi ke sheet 2
writetable(injTable, outputFilename, 'Sheet', 2, 'WriteVariableNames', true);
